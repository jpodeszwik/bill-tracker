...
const ActivitiesGrid: React.FC<PropTypes> = ({activities, offset}: PropTypes) => {
  const currentMonth = moment().add(offset, 'months').format('YYYY-MM');
  const previousMonth = moment().add(offset - 1, 'months').format('YYYY-MM');
  const nextMonth = moment().add(offset + 1, 'months').format('YYYY-MM');
